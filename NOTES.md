...
### 2026-08-09
- **Fixed the jarring full reload when moving between the homepage and a standalone page, and added a checker so it can't silently come back.** The site is two things stitched together: `/` is an SPA where About/Archive/Backstory are tab swaps inside one document, while `/22/`, `/loyalty/`, `/standard/`, `/carwash/`, `/familytree/` are separate documents. Crossing that boundary is a real navigation — the document is torn down and rebuilt, so the nav and the corner links blink out and come back. Three things were making that worse than it had to be, and all three are fixed. **(1) The stylesheet was cache-split.** `index.html` requested `styles.css?v=88` while every standalone page requested `?v=71`. Same bytes on disk, but a different URL is a different cache entry, so every crossing refetched a stylesheet the browser already had — a cold round trip landing exactly when the page was blank. Everything is on `v=89` now. **(2) The nav didn't exist in the HTML of the standalone pages.** `essay-chrome.js` built the `<header>` in JavaScript and inserted it, so the corner was genuinely empty until a script ran, while the homepage had its nav in the markup — that asymmetry is most of what read as "flimsy". The header now ships in the HTML of all five pages; `essay-chrome.js` (bumped to `v=8`) keeps only the favicon links and the footer clock, neither of which is visible furniture. **(3) The fade that was supposed to mask all this had been dead since June.** `page-transition.js` once intercepted clicks and animated `.page-enter-prep` / `.page-leaving`, but commit `313cf4e` ("Fix essay navigation transitions", 2026-06-14) stripped 88 of its 90 lines down to a no-op that only *removes* those classes — while the CSS stayed in `styles.css` doing nothing and the script stayed referenced on all six pages. Deleted the file, deleted the orphaned CSS (including its `prefers-reduced-motion` branch and the now-unused `body` transform/opacity transition), and replaced it with a CSS `@view-transition { navigation: auto }` block plus `view-transition-name: site-header` on the header, so the browser cross-fades documents natively and animates the nav as a persisting element instead of one that dies and is reborn. No JS. Firefox has no cross-document support as of 2026 and simply ignores the block, landing on the plain navigation it already had — items (1) and (2) still help there, which is why they were worth doing on their own. Reduced-motion is honored via `::view-transition-*` animation overrides. **Note the tradeoff deliberately taken:** inlining the nav reintroduces exactly the copy-paste drift `essay-chrome.js` was created to prevent, so the guarantee moved from "one source of truth" to "a check that fails loudly" — see the next entry. Bumped `styles.css` to `v=89`; also made the two asset paths in `index.html` absolute for consistency with the standalone pages.
- **Added `check-site.sh` and `drafts/essay-template.html` — the infrastructure answer to "will this break again when I add an Archive entry?"** It would have, because every standalone page is a hand-maintained copy of the last one, and copying a live page copies whatever drift it had accumulated; that is literally how the `v=71` vs `v=88` split happened. With no build step there is no way to make the duplication impossible without giving up either the cache-buster or the instant-painting nav, so the fix is to make drift *loud* instead of *invisible*. `check-site.sh` fails on: stylesheet versions disagreeing across any page, `essay-chrome.js` versions disagreeing, the `<header>` block differing from the template on any page, any relative `src`/`href` inside a page directory (a relative `styles.css` resolves to `/22/styles.css` and 404s into an unstyled page), any internal link missing its trailing slash (costs a 301 hop), and any link to a retired `.html` path. Verified by deliberately breaking a page three ways — all three were caught, and it went green again on revert. `drafts/essay-template.html` is now the only correct starting point for a new page; `drafts/` is repo-only and unlinked from the site. Documented both in `AGENTS.md`, replacing the stale flat-filename convention. Repo tooling only, nothing visitor-facing.
