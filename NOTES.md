...
### 2026-08-20
- **Bio's third work item: "AI-enabled radiology" → "internal tooling for radiology."** Amends the paragraph rewrite logged further down this day — that bullet's quoted "Now:" text is left as-is, since it's an accurate record of what shipped in `057a3ee`. The old phrase named the domain and let "AI-enabled" imply the work; the new one names what was actually built and drops the adjective. It also stops the sentence from putting "AI" in two consecutive items ("consumer AI products, AI-enabled radiology"), which read as a theme rather than as two separate jobs. Slightly longer and less punchy, which is the trade — it's the more accurate description. Copy-only change in `index.html`, no CSS/JS, so no cache-buster bump.
- **Nav clicks no longer animate — switching views is instant. The only motion left on the site is the first-load entrance.** Home ↔ Bio ↔ Archive used to cross-fade the home layer over 0.5s while the section slid in from the right over 0.4s (`translate3d(100%,0,0)` → `0`, opacity 0 → 1). All three transition declarations are gone: one on `.home-view`, one on `.home-view.hidden`, one on the shared `.about-view, .experience-view, .projects-view, .essays-view, .essay-view, .interests-view` rule. The class flips in `navigate()` all happen in the same synchronous block, so with nothing to interpolate the old layer hides and the new one paints on the same frame. **What stays is the entrance:** `.name-box` fades in over 1.8s and `header, footer` over 1.05s after a 1.3s delay, both `@keyframes fadeIn` animations rather than transitions, so removing transitions didn't touch them. That's now the whole of it — you get the animation once, when you arrive, and never again while moving around.
- Kept `transform: translate3d(100%, 0, 0)` and `will-change: transform, opacity` on the hidden views even though nothing animates them now. **The `will-change` is load-bearing for reasons unrelated to motion:** the comment on `.about-content` (~line 520) notes that a composited ancestor is what keeps that scroll container alive on iOS, and `translate3d` + `will-change` are what composite it. Dropping them to tidy up would risk the dead-scroll bug that comment exists to prevent.
- `.booting`'s `transition: none !important` block is now **inert** and labeled as such in the file — it suppresses transitions that no longer exist. Left in place, along with the `booting` class on `<html>` and the `classList.remove('booting')` in `site.js`, because it's exactly the guard that has to be there if view switching is ever animated again, and removing it would mean touching two more files and a `site.js` bump for zero visitor-visible gain. Also rewrote the stale half of the `.home-view.hidden` comment: `visibility: hidden` no longer waits out a fade duration, it applies on the same frame as `opacity: 0`. The hit-testing warning in that comment still stands and is the reason `visibility` is there at all.
- The `body` transition at the top of the file (`html.page-enter-prep` / `html.page-leaving`) was **not** touched. It's for cross-document navigation, not the SPA nav, and it has been unreachable since `313cf4e` (2026-06-14) — `page-transition.js` only ever clears those classes, never sets them. Separate question from this change. Bumped `styles.css` to `v=90` across all six pages.
